...
async function renderMovies() {
    ...
    movies.forEach (movie => {
        creatrSeparateMovieTabs(movie, movieHolder);
    });
...
function creatrSeparateMovieTabs(data, wrapper) {
    let a = document.createElement('a');
    a.setAttribute('data-movie-name', `${data.name}`);
    a.classList.add('movie-link');
    a.href = `#${data.name}`;
    a.innerHTML = `<dev class="movie" data-id=${data.name}>
